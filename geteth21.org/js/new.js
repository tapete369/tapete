...
//var one_line_w = 55;
if ($.cookie("one_line_w_eth") != null) {
  one_line_w = $.cookie("one_line_w_eth");
} else {
  one_line_w = $("#progress .circle").attr("stroke-dasharray").replace(", 100", "");
}

if ($.cookie("num_line_eth") != null) {
  num_line = $.cookie("num_line_eth");
  if (num_line < 100) {
    //one_line_w = $('#progress .circle').attr('stroke-dasharray').replace(', 100', '');
    num_line = 30000;
    Toast.add({
...
function line_progress() {
  ...
  $.cookie("num_line_eth", num_line);
  $.cookie("one_line_w_eth", one_line_w);
}
line_progress();
